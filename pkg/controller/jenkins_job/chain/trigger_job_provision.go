...
func (h TriggerJobProvision) triggerJobProvision(jj *v2v1alpha1.JenkinsJob) error {
	...
	pn, err := h.getParamFromJenkinsJobConfig("PIPELINE_NAME", jj.Spec.Job)
	if err != nil {
		return errors.Wrapf(err, "an error has been occurred while getting parameter PIPELINE_NAME from Jenkins job config")
	}
	sn, err := h.getParamFromJenkinsJobConfig("STAGE_NAME", jj.Spec.Job)
	if err != nil {
...
func (h TriggerJobProvision) getParamFromJenkinsJobConfig(name string, jj v2v1alpha1.Job) (*string, error) {
	jobConfig := make(map[string]string)
	err := json.Unmarshal([]byte(jj.Config), &jobConfig)
	if err != nil {
		return nil, err
	}

...
func (h PutCDPipelineJenkinsFolder) tryToSetCDPipelineOwnerRef(jf *v1alpha1.JenkinsFolder) error {
	...
	p, err := h.getCdPipeline(jf.Name, jf.Namespace)
	if err != nil {
		return errors.Wrapf(err, "couldn't get CD Pipeline %v from cluster", jf.Name)
	}
...
func (h PutCDPipelineJenkinsFolder) ServeRequest(jf *v1alpha1.JenkinsFolder) error {
	...
	if err := jc.CreateFolder(fmt.Sprintf("%v-%v", jf.Name, "cd-pipeline")); err != nil {
		return err
	}

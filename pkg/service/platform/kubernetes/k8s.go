...
// CreateHelmRole creates jenkins-helm role in kube-system namespace
func (service K8SService) CreateHelmRole(instance v1alpha1.Jenkins) error {
	...
	if err := controllerutil.SetControllerReference(&instance, ro, service.Scheme); err != nil {
		return errors.Wrap(err, "Failed to set Owner Reference")
	}

	r, err := service.authClient.Roles(ro.ObjectMeta.Namespace).Get(ro.ObjectMeta.Name, metav1.GetOptions{})
	if err == nil {
		return err
	}

	if !k8sErrors.IsNotFound(err) {
		return errors.Wrapf(err, "Failed to get %v role!", r.Name)
	}

	r, err = service.authClient.Roles(ro.Namespace).Create(ro)
	if err != nil {
		return errors.Wrapf(err, "Failed to create Role %v", ro.Name)
	}
	log.Info("Role has been created.",
		"Namespace", r.Namespace, "Name", r.Name, "RoleName", r.Name)

	return nil
}

func (service K8SService) CreateHelmRoleBinding(instance v1alpha1.Jenkins) error {
	labels := helper.GenerateLabels(instance.Name)
	...
	if err := controllerutil.SetControllerReference(&instance, rbo, service.Scheme); err != nil {
		return err
	}
	...
	if !k8sErrors.IsNotFound(err) {
		return errors.Wrapf(err, "Failed to get Role %v", rbo.Name)
	}

	rb, err = service.authClient.RoleBindings(rbo.Namespace).Create(rbo)
	if err != nil {
		return errors.Wrapf(err, "Failed to create Role Binding %v", rbo.Name)
	}
	log.Info("Role Binding has been created",
		"Namespace", rb.Namespace, "Name", rb.Name, "RoleBindingName", rb.Name)

...
func (j JenkinsServiceImpl) Configure(instance v1alpha1.Jenkins) (*v1alpha1.Jenkins, bool, error) {
	...
	for _, template := range []string{kanikoTemplateName, dockerRegistryTemplateName, cbisTemplateName} {
		configMapKey := template
		configMapName := fmt.Sprintf("%v-%v", instance.Name, strings.Split(template, ".")[0])
		filePath := fmt.Sprintf("%s/%s", templatesDirectoryPath, template)
		err = j.platformService.CreateConfigMapFromFileOrDir(instance, configMapName, &configMapKey, filePath, &instance)
		if err != nil {
